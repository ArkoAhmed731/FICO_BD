...
# Remove unwanted characters (underscores) and convert to numeric.
...
# =====================================
# Map the categorical target "Credit_Score" to numeric values.
mapping = {
    "Poor": 350,
    "Fair": 550,
    "Good": 650,
...
# Drop rows where mapping failed (target is NaN).
...
# =====================================
# 4. Explicit Binning of Numeric Features on Training and Testing Data
# =====================================
# After filtering the training data, recompute the binned feature matrix.
# Here, we use KBinsDiscretizer to bin each numeric feature into 10 quantile-based bins.
binner = KBinsDiscretizer(n_bins=10, encode='ordinal', strategy='quantile')
df_train_binned = pd.DataFrame(binner.fit_transform(df_train[numeric_features]),
                               columns=numeric_features)
df_test_binned = pd.DataFrame(binner.transform(df_test[numeric_features]),
                              columns=numeric_features)

# Use the binned values as our numeric feature matrix.
X_train_numeric = df_train_binned.values
X_test_numeric = df_test_binned.values
...
p = X_train_numeric.shape[1]  # number of binned numeric features

for leaf in unique_leaves:
    indices = np.where(train_leaf_indices == leaf)[0]
    X_leaf = X_train_numeric[indices]
    y_leaf = y_train[indices]
    # Build the additive term structure by starting with s(0) and adding s(1) through s(p-1).
    terms = s(0)
    for i in range(1, p):
        terms = terms + s(i)
...
# =====================================
# 7. Predict on Test Data Using the TGAMT Approach
# =====================================
...
final_df.to_csv("final_credit_scores_tgam_binning.csv", index=False)
print("Final TGAMT output saved to final_credit_scores_tgam_binning.csv")
