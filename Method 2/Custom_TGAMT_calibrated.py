...
# Use only columns that exist in the training data.
...
# =====================================
# Map the categorical "Credit_Score" to numeric values.
mapping = {
    "Poor": 350,
    "Fair": 550,
    "Good": 650,
...
for leaf in unique_leaves:
    ...
    # Create an additive term structure by starting with s(0) and adding s(1)...s(p-1)
    terms = s(0)
    for i in range(1, p):
        terms = terms + s(i)
...
# =====================================
# 7. Predict on Training Data for Calibration
# =====================================
train_preds = np.zeros(len(X_train_numeric))
for leaf in unique_leaves:
    indices = np.where(train_leaf_indices == leaf)[0]
    if len(indices) > 0:
        train_preds[indices] = leaf_gams[leaf].predict(X_train_numeric[indices])
# Clip predictions and round.
train_preds = np.clip(train_preds, 350, 850)
train_preds = np.round(train_preds).astype(int)
print("Mean TGAMT training prediction:", train_preds.mean(), "vs. actual mean:", y_train.mean())

# Fit a calibration model to map raw TGAMT predictions to the actual target.
calibrator = LinearRegression()
calibrator.fit(train_preds.reshape(-1, 1), y_train)

# =====================================
# 8. Predict Using the TGAMT Approach on Test Data
# =====================================
X_test_numeric = imputer.transform(df_test[numeric_features])
test_leaf_indices = tree.apply(X_test_numeric)
test_preds = np.zeros(len(X_test_numeric))
for leaf in unique_leaves:
    indices = np.where(test_leaf_indices == leaf)[0]
    if len(indices) > 0:
        test_preds[indices] = leaf_gams[leaf].predict(X_test_numeric[indices])
# Apply calibration to test predictions.
calibrated_test_preds = calibrator.predict(test_preds.reshape(-1, 1)).flatten()
calibrated_test_preds = np.clip(calibrated_test_preds, 350, 850)
calibrated_test_preds = np.round(calibrated_test_preds).astype(int)

# =====================================
# 9. Assign Unique Customer IDs and Save Final Output
# =====================================
df_test["tgam_score"] = calibrated_test_preds
df_test["customer_id"] = ['C' + str(i).zfill(5) for i in range(1, len(df_test) + 1)]
final_df = df_test[["customer_id", "tgam_score"]]
final_df.to_csv("final_credit_scores_tgam_calibrated.csv", index=False)
print("Final TGAMT output saved to final_credit_scores_tgam_calibrated.csv")
